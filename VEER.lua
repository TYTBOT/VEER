...
if text ==('طرد الاعضاء') and is_monsh(msg) then local function ban_all(extra, result) if not msg.can_be_deleted_ == true then VEER_sendMsg(msg.chat_id_, msg.id_, 1, "*🚨 ┇* تنبيــه انا لست ادمن يرجى ترقيتي ادمن هنا 👮\n", 1, "md") else for k,v in pairs(result.members_) do local function ban_all(extra, result) if result.first_name_ then changeChatMemberStatus(msg.chat_id_, result.id_, "ban_all") end end getUser(v.user_id_, ban_all, nil) end VEER_sendMsg(msg.chat_id_, msg.id_, 0,'🗑* ┇* تم مسح الحسابات المحذوفه', 1, 'md') end end tdcli_function ({ID = "GetChannelMembers",channel_id_ = getChatId(msg.chat_id_).ID,offset_ = 0,limit_ = 1000}, ban_all, nil) end
if is_mod(msg) then 
